...
def select_current_status():
    ...
    # with closing(ph.get_conn()) as conn:
    #     cur = conn.cursor()
    #     hook.run(f'truncate table {BUFFER_TABLE_NAME}')

    #     src_cur.execute(max_cut_query)
    #     src_max = src_cur.fetchone()

    dest = PostgresHook(postgres_conn_id='postgres_otus')
    dest_conn = dest.get_conn()
    dest_cursor = dest_conn.cursor()

    dest_cursor.execute('select count(*) from btc_data;')
    print(dest_cursor.fetchone())
...
select_snth = PythonOperator(
    task_id='select_snth', python_callable=select_current_status, dag=dag)

start >> create_table_if_not_exist >> request_data >> parse_data >> insert_data >> select_snth
